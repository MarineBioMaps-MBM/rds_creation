# Creating an RDS file for the biota data for the North Central Coast Study Region (NCCSR)
...
nccsr_bio <- central_bio_mpa |>
  filter(study_regi == "NCCSR")

# Save the rds file into the outputs folder
outputs.dir <- file.path("rds_outputs")
file_path <- file.path(outputs.dir, "nccsr_biota.rds")
saveRDS(nccsr_bio, file = file_path)